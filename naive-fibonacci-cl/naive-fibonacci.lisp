...
(declaim (ftype (function (fixnum) fixnum) naive-fibonacci-recursive))
;; (declaim (inline naive-fibonacci-recursive))
;; the above breaks: *INLINE-EXPANSION-LIMIT* (50) was exceeded while inlining NAIVE-FIBONACCI-RECURSIVE
(defun naive-fibonacci-recursive (n)
  (declare (type fixnum n))
  (if (< n 2)
      n
      (the fixnum (+ (naive-fibonacci-recursive (1- n))
                     (naive-fibonacci-recursive (- n 2))))))
...
(declaim (ftype (function ((integer 0 47)) (integer 0 *)) 
                naive-fibonacci-recursive-2))
(declaim (inline naive-fibonacci-recursive-2))
(defun naive-fibonacci-recursive-2 (n)
  (declare (type (integer 0 47) n))
  (if (< n 2)
      n
      (the fixnum (+ (naive-fibonacci-recursive (1- n))
                     (naive-fibonacci-recursive (- n 2))))))
...
(defun naive-fibonacci-2 (n)
  (let* ((start-time (get-internal-real-time))
         (res (naive-fibonacci-recursive-2 n))
         (end-time (get-internal-real-time))
         (duration (/ (- end-time start-time)
                      internal-time-units-per-second)))
    (format t "Fibonacci(~D) = ~D in ~F seconds~%"
...
(defun naive-fibonacci-A (n)
  ...
  (sb-alien:define-alien-routine
      ("naive_fibonacci" c-naive-fibonacci-A)
    sb-alien:double
    (n sb-alien:unsigned-int))

  (let ((duration (c-naive-fibonacci-A n)))
    (format t "C function returned duration = ~f~%" duration)
    duration))

(defun naive-fibonacci-A--benchmark-5-times (n)
  ""
  (let ((nb-runs 5)
        (durations '()))
    (dotimes (i nb-runs)
      (let ((duration (naive-fibonacci-A n)))
        (format t "Run ~D / ~D: ~F seconds~%" (1+ i) nb-runs duration)
        (push duration durations)))
    (locally
        (declare (sb-ext:muffle-conditions sb-ext:compiler-note))
...
(defun naive-fibonacci-B (n)
  ...
  (cffi:defcfun ("naive_fibonacci" c-naive-fibonacci-B)
      :double
    (n :uint32))

  (let ((duration (c-naive-fibonacci-B n)))
    (format t "C function returned duration = ~f~%" duration)
    duration))

(defun naive-fibonacci-B--benchmark-5-times (n)
  ""
  (let ((nb-runs 5)
        (durations '()))
    (dotimes (i nb-runs)
      (let ((duration (naive-fibonacci-B n)))
        (format t "Run ~D / ~D: ~F seconds~%" (1+ i) nb-runs duration)
        (push duration durations)))
    (locally
        (declare (sb-ext:muffle-conditions sb-ext:compiler-note))
...
(defun main ()
  (let ((n 46))                         ; 1836311903
    ...
    (naive-fibonacci-2 n)
    ;; (naive-fibonacci-A n)
    ;; (naive-fibonacci-A--benchmark-5-times n)
    ;; (naive-fibonacci-B n)
