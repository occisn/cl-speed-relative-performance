...
(defparameter *n* 100000) ; 100 K
(defparameter *m* 100000) ; 100 K
(defparameter *u* 40)
(declaim (type fixnum *n* *m* *u*))

(defun loops ()
  "Perform the main benchmark loop."
  (let* ((sum 0)
...
(defun loops-3 ()
  ...
  (cffi:defcfun ("run" c-loops-3)
      :double)

  (let ((duration (c-loops-3)))
    (format t "C function returned duration = ~f~%" duration)
    duration))

(defun loops-3--benchmark-5-times ()
  ""
  (let ((nb-runs 5)
        (durations '()))
    (dotimes (i nb-runs)
      (let ((duration (loops-3)))
        (format t "Run ~D / ~D: ~F seconds~%" (1+ i) nb-runs duration)
        (push duration durations)))
    (locally
        (declare (sb-ext:muffle-conditions sb-ext:compiler-note))
